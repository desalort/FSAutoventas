...
class recibo_proveedor extends fs_model
{
   ...
   {
      parent::__construct('recibosprov', 'plugins/tesoreria/');
      if($r)
      {
   ...
   public function save()
   {
      ...
      else
      {
         $sql = "INSERT INTO ".$this->table_name." (cifnif,codproveedor,codcuenta,
            coddivisa,codigo,codpago,codserie,estado,fecha,fechav,fechap,idfactura,idremesa,iban,
            importe,importeeuros,nombreproveedor,numero,swift,tasaconv) VALUES "
   ...
   public function all_from_proveedor($codproveedor, $offset=0, $limit=FS_ITEM_LIMIT)
   {
      ...
      $sql = "SELECT * FROM ".$this->table_name." WHERE codproveedor = ".$this->var2str($codproveedor)." ORDER BY fecha DESC";
      $data = $this->db->select_limit($sql, $limit, $offset);
      if($data)
      {
         foreach($data as $d)
